  ...
  class CommitsController < BaseController
    def create
      ...
      VaultReplica.transaction do
        ...
        if (existing = EventCommit.find_by(event_record: event))
          unless ActiveSupport::SecurityUtils.secure_compare(existing.request_sha256, request_sha256)
    ...
    def commit_json(commit)
      ...
      { eventObjectId: commit.event_record.object_id,
       generationId: commit.vault_generation.generation_id,
       generationNumber: commit.vault_generation.generation_number,
       dependencyObjectIds: ids, cursor: commit.cursor, durabilityAcknowledged: true }
    end
  end
end
