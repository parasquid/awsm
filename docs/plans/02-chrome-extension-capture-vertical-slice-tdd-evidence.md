...
|---:|:---:|---|
| 1 | PASS | `vault.test.ts` covers device-only and device-plus-passphrase creation/unlock; packaged Chrome creates a device-only Vault through keyboard-operated onboarding. |
...
- Runtime preparation rejects empty, duplicate, missing, and contradictory selections before commit. The Driver commits the encrypted Event and every affected encrypted Projection row in one transaction while retaining Bundle Objects.
